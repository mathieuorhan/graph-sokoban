...
class PrioritisedReplayBuffer(MaxHeap, Deque):
    ...
    def update_heap_and_heap_index_to_overwrite(self):
        ...
        if not self.reached_max_capacity:
            self.update_heap_element(
                self.heap_index_to_overwrite_next,
                self.deque[self.deque_index_to_overwrite_next],
            )
            self.deque[
                self.deque_index_to_overwrite_next
            ].heap_index = self.heap_index_to_overwrite_next
    ...
    def pick_experiences_based_on_proportional_td_error(self):
        ...
        deque_sample_indexes = np.random.choice(
            range(len(self.deques_td_errors)),
    ...
    def calculate_importance_sampling_weights(self, experiences):
        ...
        importance_sampling_weights = [
            (
                (1.0 / self.number_experiences_in_deque)
                * (self.give_adapted_sum_of_td_errors() / td_error)
            )
            ** self.beta
        ...
        importance_sampling_weights = [
            is_weight / sample_max_importance_weight
            for is_weight in importance_sampling_weights
        ]
        importance_sampling_weights = (
            torch.tensor(importance_sampling_weights).float().to(self.device)
        )
    ...
    def update_td_errors(self, td_errors):
        ...
        for raw_td_error, deque_index in zip(
            td_errors, self.deque_sample_indexes_to_update_td_error_for
        ):
            td_error = (abs(raw_td_error) + self.incremental_td_error) ** self.alpha
