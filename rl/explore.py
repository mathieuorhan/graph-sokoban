...
def best_from_nodes(scores, state, ignore_walls=True, ignore_static=True):
    """
    Args:
    ...
    mask = state.mask.clone().detach()
    if ignore_walls:
        walls = state.x[:, 3] == 1
        mask[walls] = False
    if ignore_static:
        mask[state.player_idx] = False
...
def epsilon_greedy_only_graph(
    ...
    """
    if eps == 0 or random_generator.random() > eps:
        with torch.no_grad():
        ...
        if do_nothing:
            ...
            if nb_wall_neighbors:
                ...
                mask = neighbors_are_walls.cpu().numpy()
                weights = np.empty_like(mask, dtype=float)
                weights[mask] = wall_weight
                weights[~mask] = non_wall_weight
